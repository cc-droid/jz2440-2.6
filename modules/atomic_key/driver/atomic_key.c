...
static struct fasync_struct* atomic_key;
...
static irqreturn_t key_irq(int irq, void *dev_id){
    ...
    kill_fasync (&atomic_key, SIGIO, POLL_IN);

	return IRQ_HANDLED;
}

static int atomic_key_fasync (int fd, struct file *filp, int on)
{
    return fasync_helper (fd, filp, on, &atomic_key);
}

static unsigned int atomic_key_poll (struct file *filp, struct poll_table_struct *table){
    unsigned int mask = 0;
...
static ssize_t atomic_key_read (struct file *filp, char __user *buff, size_t count, loff_t *offset){
    int ret;
    char status;
...
static int atomic_key_open (struct inode *inode, struct file *filp){
    int ret;
    if(!atomic_dec_and_test(&opened)){
        atomic_inc(&opened);
        return -EBUSY;
    }
...
static int atomic_key_close(struct inode *inode, struct file *filp){
    atomic_inc(&opened);
    free_irq(IRQ_EINT0,&pin_descs[0]);
...
static struct file_operations key_ops = {
    .owner = THIS_MODULE,
    .open = atomic_key_open,
    .read = atomic_key_read,
    .release = atomic_key_close,
    .poll = atomic_key_poll,
    .fasync =atomic_key_fasync,
};
static struct class *key_class;
static struct class_device *key_class_device;

static int __init atomic_key_init(void){
    int ret;
...
module_init(atomic_key_init);

static void __exit atomic_key_exit(void){

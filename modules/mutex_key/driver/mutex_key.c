...
static struct fasync_struct* mutex_key;
...
static irqreturn_t key_irq(int irq, void *dev_id){
    ...
    kill_fasync (&mutex_key, SIGIO, POLL_IN);

	return IRQ_HANDLED;
}

static int mutex_key_fasync (int fd, struct file *filp, int on)
{
    return fasync_helper (fd, filp, on, &mutex_key);
}

static unsigned int mutex_key_poll (struct file *filp, struct poll_table_struct *table){
    unsigned int mask = 0;
...
static ssize_t mutex_key_read (struct file *filp, char __user *buff, size_t count, loff_t *offset){
    int ret;
    char status;
...
static DECLARE_MUTEX(key_mutex);
static int mutex_key_open (struct inode *inode, struct file *filp){
    int ret;
    down_interruptible(&key_mutex);
...
static int mutex_key_close(struct inode *inode, struct file *filp){
    up(&key_mutex);
    free_irq(IRQ_EINT0,&pin_descs[0]);
...
static struct file_operations key_ops = {
    ...
    .open = mutex_key_open,
    .read = mutex_key_read,
    .release = mutex_key_close,
    .poll = mutex_key_poll,
    .fasync =mutex_key_fasync,
};
static struct class *key_class;
static struct class_device *key_class_device;

static int __init mutex_key_init(void){
    int ret;
...
module_init(mutex_key_init);

static void __exit mutex_key_exit(void){

...
static irqreturn_t key_irq(int irq, void *dev_id){
    ...
    printk(KERN_INFO"%d, irq\n", irq);
	return IRQ_HANDLED;
}

static unsigned int interrupt_key_poll (struct file *filp, struct poll_table_struct *table){
    unsigned int mask = 0;
...
static ssize_t interrupt_key_read (struct file *filp, char __user *buff, size_t count, loff_t *offset){
    int ret;
    char status;
...
static int interrupt_key_open (struct inode *inode, struct file *filp){
    int ret;
...
static int interrupt_key_close(struct inode *inode, struct file *filp){
...
static struct file_operations key_ops = {
    .owner = THIS_MODULE,
    .open = interrupt_key_open,
    .read = interrupt_key_read,
    .release = interrupt_key_close,
    .poll = interrupt_key_poll,
};
